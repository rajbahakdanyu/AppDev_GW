...
namespace AppDev_GW
{
    public partial class OutofStock : System.Web.UI.Page
    {
        ...
        private void BindGrid(string expression = "[Id]", string direction = "ASC")
        {
            ...
            String queryString = $"SELECT i.[Id], i.[Name], i.[Description], s.[Quantity] FROM Item as i JOIN [Stock] s ON s.ItemId = i.Id WHERE s.[Quantity] = 0 ORDER BY {expression} {direction} ";
