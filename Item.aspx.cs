...
namespace AppDev_GW
{
    public partial class About : Page
    {
        ...
        protected void btnAddItem_Click(object sender, EventArgs e)
        {
            ...
            string itemMenufactureDate = txtItemManufactureDate.Text.ToString() ?? "";
            string itemExpiryDate = txtItemExpiryDate.Text.ToString() ?? "";
            String queryString = "";
            if (btnAddItem.Text == "Insert")
            {
                queryString = $"INSERT INTO ITEM (NAME,DESCRIPTION,PRICE,PURCHASE,CATAGORY,MANUFACTURE,EXPIRY) VALUES('{itemName}','{itemDescription}',{itemPrice}, TO_DATE('{itemPurchaseDate}','dd/mm/yyyy')',{itemCatagory}, TO_DATE('{itemMenufactureDate}','dd/mm/yyyy')', TO_DATE('{itemExpiryDate}','dd/mm/yyyy')')";
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                //con.Open();
                //cmd.ExecuteReader();
                Response.Write($"<script language=javascript>alert('{btnAddItem.Text}')</script>");
            }
        ...
        protected void OnRowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("DELETE FROM ITEM WHERE ID ='" + ID + "' "))
                {
